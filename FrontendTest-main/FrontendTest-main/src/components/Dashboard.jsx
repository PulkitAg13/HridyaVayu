...
const [inhalerData, setInhalerData] = useState(null);
  ...
  // Function to invoke upload sensor data
  const uploadSensorData = async () => {
    ...
    // Fetch user's location from browser
        ...
        async (position) => {
          ...
          try {
            const response = await axios.post(
              "http://127.0.0.1:5000/upload-sensor-data",
              {
                user_id,
            ...
            alert("Environment data uploaded successfully!");
            console.log("Response:", response.data);
          } catch (error) {
            console.error("Error uploading environment data:", error);
            alert("Failed to upload environment data!");
          }
        },
        (error) => {
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen">
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white p-3 flex justify-around shadow-lg">
        ...
        <button onClick={() => navigate("/find-my-device")}>
          <AiOutlineEnvironment className="text-2xl text-gray-600" />
        </button>
        <button onClick={() => navigate("/alert-history")}>
